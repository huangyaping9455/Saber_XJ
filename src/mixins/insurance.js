                ...
                if (val != "" && val != undefined && val != null) {
                    this.FIELD.cheliangpaizhao.dicData.map(el => {
                        if (el.cheliangpaizhao === val) {
                            ...
                            this.form.shiyongxingzhi = el.shiyongxingzhi;
    ...
    methods: {
        ...
        getField() {
            return getField(this.CONFIG.viewField, this.deptId).then((res) => {
                // this.FIELD = Object.assign(this.buildFieldMap(res.data.data), this.FIELDMinXi);
                this.FIELDMSG = this.buildFieldMap(res.data.data);
            });
        },
        // 初始化保险明细
        INITMinXi(MinXitoken) {
            return this.getConfigMinXi(MinXitoken).then(this.getFieldMinXi);
        },
        // 获取保险明细配置
        getConfigMinXi(MinXitoken) {
            return getConfig(MinXitoken).then(res => {
                this.CONFIGMinXi = res.data.data;
            })
        },
        // 获取保险明细字段
        getFieldMinXi() {
            return getField(this.CONFIGMinXi.viewField, this.deptId).then((res) => {
                this.FIELDMinXi = this.buildFieldMap(res.data.data);
                this.FIELD = Object.assign(this.FIELDMSG, this.FIELDMinXi);
            });
        },
        // 判断是否为 数字
        isNumber(val) {
        ...
        rowSave(row = this.vehicleSafeData, callback, list = true) {
            ...
            insert(this.CONFIG.insertModel, this.sendHandle(row)).then((res) => {
                ...
                let baoxianId = res.data.data;
                row.baoxianid = baoxianId;
                row.deptId = row.deptName;
                insert(this.CONFIGMinXi.insertModel, this.sendHandle(row)).then((res) => {
                    this.$message({
                        type: "success",
                        message: res.data.msg,
                    });
        ...
        rowUpdate(row, index, callback, list = true) {
            row.isdelete = 0
            update(this.CONFIG.updateModel, this.sendHandle(row)).then((res) => {
                row.id = row.mingxiid;
                update(this.CONFIGMinXi.updateModel, this.sendHandle(row)).then((res) => {
                    this.$message({
                        type: "success",
                        message: res.data.msg,
                    });
